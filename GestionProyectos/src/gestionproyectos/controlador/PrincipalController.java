...
public class PrincipalController {
    ...
    TareaInfoJpaController CTareasInfo = new TareaInfoJpaController();
    ...
    // Este es el id de la persona que se conecto a la aplicacion
    private int idPersonaConectada;
    // Este es el instante de timempo inicio cuando realizo una tarea
    private int tiempoInicio1;
    ...
    private List <TareaInfo> infoTareasSolic;
    private int tiempoTotaltarea;
    private List <TareaInfo> todastareasinfo;
    ...
    public void asociarTareaInfo(String text) {
    
    int num = this.tareaRealizar.getIdTarea();
    int num2 = this.idPersonaConectada;
    
    TareaInfo tareaInfo = new TareaInfo();
    tareaInfo.setDescripcion(text);
    tareaInfo.setIdtarea(num);
    tareaInfo.setIdusuario(num2);
    tareaInfo.setTiempoTotal((tiempoFinal1 - tiempoInicio1)/ 1000);
   
    CTareasInfo.create(tareaInfo);
...
return desc;
    ...
    public void setIdPersonaConectada(int idPersonaConectada) {
        this.idPersonaConectada = idPersonaConectada;
    }

    
    public void tiempoInicio(int tiempoInicio){
    
    this.tiempoInicio1=tiempoInicio   ; 
    
    }
    
    public void tiempoFinal(int tiempoFinal){
    
    this.tiempoFinal1=tiempoFinal   ; 
    
    }
    
    public List<TareaInfo> getTotasTareasInfo(){
    
    this.todastareasinfo=CTareasInfo.findTareaInfoEntities();
    return this.todastareasinfo;
    }

    public List<TareaInfo> getTareasInfo() {
    
    
    List<TareaInfo> tareasInfo=CTareasInfo.findTareaInfoEntities();
    List<TareaInfo> tar = new ArrayList<TareaInfo>();
    
    
        for (int i = 0; i < tareasInfo.size(); i++) {
            
        if(this.tareaCon.getIdTarea()==tareasInfo.get(i).getIdtarea()) {
            
        TareaInfo e = new TareaInfo();
        e.setDescripcion(tareasInfo.get(i).getDescripcion());
        e.setIdusuario(tareasInfo.get(i).getIdusuario());
        e.setTiempoTotal(tareasInfo.get(i).getTiempoTotal());
        tar.add(e);
        
        }   
            
        }
        this.infoTareasSolic=tar;
        return this.infoTareasSolic;
    ...
    public String conseguirNombre(Integer idusuario1) {
    
    List<Empleados> empleados = CEmpleados.findEmpleadosEntities();
        for (int i = 0; i < empleados.size(); i++) {
            
            if( empleados.get(i).getId()==idusuario1){
            this.nom = empleados.get(i).getNombre();
            }
    ...
    public int getTiempoTotalTarea() {
        this.tiempoTotaltarea=0;        
        for (int i = 0; i < this.infoTareasSolic.size(); i++) {
            
        
           this.tiempoTotaltarea=this.tiempoTotaltarea + this.infoTareasSolic.get(i).getTiempoTotal();
        }
     return this.tiempoTotaltarea;
    }
